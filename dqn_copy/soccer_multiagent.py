...
import argparse
import pandas as pd
from environments.soccer.env import Soccer
...
def get_name_brain(args, idx):
    ...
    return './results_soccer/weights_files/' + file_name_str + '_' + str(idx) + '.h5'
...
def get_name_rewards(args):
    ...
    return './results_soccer/rewards_files/' + file_name_str + '.csv'
...
def get_name_timesteps(args):
    ...
    return './results_soccer/timesteps_files/' + file_name_str + '.csv'
...
class Environment(object):
    ...
    def __init__(self, arguments):
        ...
        self.env = Soccer(arguments, current_path)
    ...
    def run(self, agents, file1, file2):
        ...
        for episode_num in range(self.episodes_number):
            ...
            while not done and time_step < self.max_ts:
                ...
                next_state, reward, done, result = self.env.step(actions)
                # converting list of positions to an array
            ...
            wins[episode_num] = result

            print("Episode {p}, Score: {s}, Final Step: {t}, Goal: {g}".format(p=episode_num, s=reward_all,
                                                                               t=time_step, g=done))
            if self.recorder:
                os.system("ffmpeg -r 4 -i ./results_soccer/snaps/%04d.png -b:v 40000 -minrate 40000 -maxrate 4000k -bufsize 1835k -c:v mjpeg -qscale:v 0 "
                          + "./results_soccer/videos/{a1}_{a2}_{a3}_{a4}_{a5}.avi".format(a1=self.num_predators,
                                                                                            a2=self.num_preys,
                                                                                            a3=self.preys_mode,
                                                                                            a4=self.game_mode,
                                                                                            a5=self.grid_size))

                files = glob.glob('./results_soccer/snaps/*')
                for f in files:
                    os.remove(f)
...
if __name__ =="__main__":
    ...
    # Game Parameters
    parser.add_argument('-k', '--agents-number', default=1, type=int, help='The number of agents')
    ...
    wins = env.run(all_agents, rewards_file, timesteps_file)
    env.plotResult(wins)
